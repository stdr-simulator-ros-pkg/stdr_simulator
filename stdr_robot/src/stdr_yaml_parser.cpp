  ...
  namespace parser {
    ...
    **/
    void operator >> (const YAML::Node& node, stdr_msgs::RobotMsg& msg) 
    {
      ...
      for (YAML::Iterator it = node["sensors"].begin(); 
        it != node["sensors"].end(); ++it) 
      {
    ...
    **/
    void operator >> (const YAML::Node& node, stdr_msgs::LaserSensorMsg& msg) 
    {
      ...
      if (msg.maxRange <= 0)
        throw YAML::RepresentationException(node["max_range"].GetMark(), 
          "negative or zero value");
      node["min_range"] >> msg.minRange;
      if (msg.minRange < 0)
        throw YAML::RepresentationException(node["min_range"].GetMark(), 
          "negative or zero value");
      node["num_rays"] >> msg.numRays;
      if (msg.numRays <= 0)
        throw YAML::RepresentationException(node["num_rays"].GetMark(), 
          "negative or zero value");
      ...
      if (msg.frequency <= 0)
        throw YAML::RepresentationException(node["frequency"].GetMark(), 
          "negative or zero value");
